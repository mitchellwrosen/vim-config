...
local function _10_(opts)
  ...
  return vim.api.nvim_create_autocmd("BufWinEnter", {once = true, buffer = opts.buf, callback = _11_})
end
vim.api.nvim_create_autocmd("BufRead", {callback = _10_})
local function _13_()
  vim.bo.modifiable = not vim.bo.readonly
  return nil
end
vim.api.nvim_create_autocmd("BufReadPost", {callback = _13_, group = "mitchellwrosen"})
local function _14_()
  return vim.highlight.on_yank({higroup = "Visual", timeout = 600})
end
vim.api.nvim_create_autocmd("TextYankPost", {callback = _14_, group = "mitchellwrosen"})
local function _15_()
...
vim.api.nvim_create_autocmd({"CursorHold", "FocusGained"}, {callback = _15_, group = "mitchellwrosen"})
local function _17_()
...
vim.api.nvim_create_autocmd({"InsertLeave", "TextChanged"}, {callback = _17_, group = "mitchellwrosen"})
local function _19_()
  return vim.cmd("startinsert")
end
vim.api.nvim_create_autocmd("TermOpen", {callback = _19_, group = "mitchellwrosen"})
do
  local lsp = require("lspconfig")
  local status = require("lsp-status")
  local capabilities
  local function _20_(config)
    _G.assert((nil ~= config), "Missing argument config on fennel/init.fnl:528")
    local cmp_nvim_lsp = require("cmp_nvim_lsp")
    return cmp_nvim_lsp.update_capabilities(vim.tbl_extend("keep", (config.capabilities or {}), status.capabilities))
  end
  capabilities = _20_
  local on_attach
  local function _21_(client, buf)
    _G.assert((nil ~= buf), "Missing argument buf on fennel/init.fnl:540")
    _G.assert((nil ~= client), "Missing argument client on fennel/init.fnl:540")
    ...
    meaningful_head = _22_
    local filter
    do
      local _24_ = vim.bo.filetype
      if (_24_ == "haskell") then
    ...
    local function _29_()
      if (vim.api.nvim_get_mode().mode == "n") then
        ...
        local function _31_(_err, result, _ctx, _config)
        ...
        return vim.lsp.buf_request(0, "textDocument/hover", position, _31_)
      else
        return nil
      end
    end
    return vim.api.nvim_create_autocmd({"CursorMoved"}, {buffer = buf, callback = _29_, group = augroup_name})
  end
  on_attach = _21_
  ...
  local function _35_(client, buf)
  ...
  lsp.elmls.setup({capabilities = capabilities(lsp.elmls), on_attach = _35_})
...
local function run_floating(command)
  ...
  vim.fn.termopen(command, {on_exit = _38_})
